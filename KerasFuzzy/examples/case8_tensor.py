...
#%%
x = Amplitude_line1[15000:16000]
xx = Amplitude_line3[15000:16000]
cx=[(c-s) for c, s in zip(x[1:], x[:-1])]
cxx=[(c-s) for c, s in zip(xx[1:], xx[:-1])]

plt.plot(cx) 
plt.title('Tensor data')
plt.xlabel('N')
plt.ylabel('y')
plt.show()
#%%
x = Amplitude_line1[:]
xx = Amplitude_line3[:]
cx=[max(-1, min(1, (c-s)/(c+s))) for c, s in zip(x[1:], x[:-1])]
cxx=[max(-1, min(1, (c-s)/(c+s))) for c, s in zip(xx[1:], xx[:-1])]
...
f_layer = FuzzyLayer(fuzzy_kernels, initial_centers=np.transpose(np.array([x_train[i] for i in indices])), 
                     input_dim = 2 * slice)
