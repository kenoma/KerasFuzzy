...
class FAE(keras.Model):
    ...
    def train_step(self, data):
        with tf.GradientTape() as tape:
            z_mean, z_log_var, z, fz = self.encoder(data)
            ...
            expect = tf.where(
                tf.equal(tf.reduce_max(fz, axis=1, keepdims=True), fz), 
                tf.constant(1.0, shape=fz.shape), 
                tf.constant(0.0, shape=fz.shape)
            )
            fz_loss = K.mean(K.categorical_crossentropy(expect, fz), axis=-1)
...
fae.compile(optimizer=keras.optimizers.Adam(lr=0.001))
...
def plot_label_clusters(fae, data, labels):
    ...
    z_classes = fae_class.predict(data)
    ...
    for odim in range(latent_dim - 1):
        ax[int(odim/2), odim % 2].scatter(z_means[:, 0], z_means[:, odim + 1], c=[ (-1 if a[0] == a[1] else a[0]) for a in zip(labels, z_classes)])
        ax[int(odim/2), odim % 2].set_xlabel("z[0]")
        ax[int(odim/2), odim % 2].set_ylabel(f"z[{odim + 1}]")
    
    plt.show()

plot_label_clusters(fae, X_train, [np.argmax(a) for a in Y_train])
