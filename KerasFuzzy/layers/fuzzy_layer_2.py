...
class FuzzyLayer2(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        if self.initial_centers is None:
            c_init= tf.random_uniform_initializer(-1, 1)(shape=(self.output_dim, input_shape[-1]), dtype="float32")    
        else:
        ...
        for diag_scale in tf.unstack(tf.linalg.diag(s_init)):
            translate_and_rotate.append(tf.concat([diag_scale, tf.reshape(c_init[index,:],(input_shape[-1],1))], 1))
            index = index + 1
